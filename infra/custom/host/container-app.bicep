...
@description('When set, redis service binding will be added to container app')
...
module checkRoleAssignment '../security/check-role-assignment.bicep' = {
  name: 'check-containerRegistryAccess'
  params: {
...
resource app 'Microsoft.App/containerApps@2023-04-01-preview' = {
  ...
  // module checkRoleAssignment will fail if role assignement does not exist
  dependsOn: usePrivateRegistry ? [ checkRoleAssignment ] : []
  identity: {
  ...
  properties: {
    ...
    template: {
      serviceBinds: enableRedisDevService ? [
        {
          name: 'redis'
          serviceId: redisDev.id
        }
      ] : [
        {
          name: 'redis'
          serviceId: redisProd.id
        }
      ]
      containers: [
        {
...
resource redisDev 'Microsoft.App/containerApps@2023-04-01-preview' = if (enableRedisDevService) {
  name: 'redis-dev'
  location: location
  tags: tags
  dependsOn: usePrivateRegistry ? [ checkRoleAssignment ] : []
...
resource redisProd 'Microsoft.Cache/redis@2023-05-01-preview' existing = if(!enableRedisDevService) {
  name: redisProdName 
}
